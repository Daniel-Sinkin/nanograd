...
class Neuron:
    def __init__(self, n_args: int = 2, seed=RNG_SEED):
        ...
        self.w: list[NanoTensor] = [
            NanoTensor(float(x)) for x in _rng.normal(1, 0.01, n_args)
        ]
        self.b = NanoTensor(float(_rng.normal(1, 0.01, 1)[0]))
...
class Layer:
    ...
    def __call__(self, x: NanoTensor) -> list[NanoTensor]:
        return [n(x) for n in self]
...
class MLP:
    ...
    def forward(self, x: NanoTensor) -> list[NanoTensor]:
        for layer in self._layers:
            x: list[NanoTensor] = layer(x)
        return x
